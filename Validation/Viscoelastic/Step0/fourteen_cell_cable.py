...
from VertexTissue.util import first, shortest_edge_length_and_time
...
def run(force, visco=False, cable=True, phi0=1.0, ndim=3):
    ...
    integrate(5, 3000, 
            dt_init = 1e-3,
            adaptive=True,
            dt_min=5e-3 if ndim==3 else 5e-2,
            save_rate=50,
...
def shortest_length(d):
    ...
    # edge_view(G)
    lmin,_=shortest_edge_length_and_time(d)
    return lmin
...
def plot_results(forces,results):
    ...
    lens = np.array(results/const.l_apical,dtype=float)
    ...
    plt.tight_layout()
    plt.savefig('fourteen_cell_belt_contraction.pdf',dpi=200)
    plt.show()
...
def main():
    forces=np.array([*np.linspace(0, 1200, 40), *np.linspace(1200,1600, 21)[1:] ])
    ...
    if viewable:
        results=parameter_sweep(forces, funcs, pre_process=shortest_length, savepaths=savepaths, overwrite=False, cache=True, inpaint=np.nan)
