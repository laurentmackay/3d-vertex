...
try:
    ...
    viewable=True
    base_path = './data/SAC+127_peripheral/'
except:
    viewable=False
...
def run(force, visco=False, cable=True, phi0=1.0, level=0, arcs=1):
    ...
    #
    squeeze = SG.arcs_and_intercalation(G, (belt,), t_belt=0, inter_edges=(inter_edges[level],),  t_intercalate=0, intercalation_strength=force, arc_strength=belt_strength if cable else 0.0)
...
def plot_results(forces,results):
    ...
    np.save(f'viscoelastic_peripheral_cable_advantange_127_level_{lvl}.npy', alpha_cable )
    np.save(f'viscoelastic_peripheral_advantange_127_level_{lvl}.npy', alpha_VE )
...
def main():
    ...
    # run(forces[-2], visco=True, phi0=0.3, cable=True, level=lvl)
    print('done')
