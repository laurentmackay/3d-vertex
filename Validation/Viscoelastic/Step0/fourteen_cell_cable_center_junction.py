...
from VertexTissue.util import first, shortest_edge_network_and_time

try:
    from VertexTissue.PyQtViz import edge_view
    import matplotlib.pyplot as plt
    viewable=True
    base_path = './data/SAC+center/'
except:
    viewable=False
...
def run(force, visco=False, cable=True, phi0=1.0):
    ...
    squeeze = SG.belt_and_intercalation(G, belt, t_belt=0, inter_edges=((11,1),),  t_intercalate=0, intercalation_strength=force, belt_strength=belt_strength if cable else 0.0)
...
def plot_buckled(forces,results):
    ...
    lbls=['uncontracted','buckled+uncontracted','contracted']
    ...
    plt.savefig('belt_center_junction_buckling.pdf',dpi=200)
...
def plot_results(forces,results):
    ...
    plt.tight_layout()
    plt.savefig('belt_center_junction_contraction.pdf',dpi=200)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('belt_center_junction_advantange.pdf',dpi=200)
    plt.show()
    ...
    alpha_VE = (phi_elastic-phi_no_cable)/phi_elastic
    alpha_cable = (phi_no_cable-phi_cable)/phi_elastic
    ...
    plt.legend(fontsize=14)
    plt.tight_layout()
    plt.savefig('visco_cable_center_advantange.pdf',dpi=200)
    plt.show()
...
# phi0_disp=[0.3, .45, .5, .53, .54, .55, .56, .59, 0.6, .7, .8,  .9]
...
def main():
    ...
    if viewable:
        results =parameter_sweep(forces, funcs, pre_process=collapsed_or_buckled, savepaths=savepaths, overwrite=False, inpaint=np.nan, cache=False)
        plot_buckled(forces, results)
    else:
