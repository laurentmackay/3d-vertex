...
try:
    ...
    viewable=True
    base_path = './data/junction_127/'
except:
    viewable=False
    base_path = '/scratch/st-jjfeng-1/lmackay/data/junction_127/'
...
def run(force, visco=False,  phi0=1.0, level=lvl, arcs=2):
...
# 
    ...
    squeeze = SG.just_intercalation(G, (inter_edges[3],),  t_intercalate=0, intercalation_strength=force)
...
def plot_results(forces,results):
    ...
    plt.xlim([0,2])
    plt.xlabel('$-\phi$ (junction)', fontsize=14)
    plt.ylabel('Contracting Edge Length (norm.)', fontsize=14)
    plt.legend()
    plt.tight_layout()

    plt.savefig(f'junction_contraction_level_{lvl}_127.pdf',dpi=200)
    plt.show()
    ...
    # plt.xlabel('$-\phi$ (cable)', fontsize=14)
    # plt.ylabel('Shortest Edge Time', fontsize=14)
    # plt.legend()
    # plt.tight_layout()

    # plt.savefig(f'junction_contraction_time_127.pdf',dpi=200)
    # plt.show()
