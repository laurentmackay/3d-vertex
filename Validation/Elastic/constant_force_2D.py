...
def run(dt):
    ...
    #create integrator
    integrate = monolayer_integrator(G, G, pre_callback=forcing, ndim=2, player=False, save_rate=1)
    t_final=600
    #integrate
    if fmag>=0:
        integrate(dt, t_final, save_pattern=f'data/elastic/extension_{fmag}_dt_{dt}_*.pickle')
    else:
