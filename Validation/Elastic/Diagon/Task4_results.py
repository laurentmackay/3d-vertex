...
plt.tight_layout()
plt.legend()
fig1.savefig(f'Task4_validation_delta1.png', dpi=200)
...
plt.tight_layout()
plt.legend()
fig2.savefig(f'Task4_validation_epsilon.png', dpi=200)


plt.figure(3)
plt.plot(phi, theo_l3,'--',label='Unbuckled Prediction',linewidth=linewidth)
plt.xlabel('$\phi$', fontsize=fs)
plt.ylabel(r'Network Height (normalized)', rotation='vertical', fontsize=fs, labelpad=12)
plt.tight_layout()
plt.legend()

fig3.savefig(f'Task4_validation_height.png', dpi=200)
