...
fmags = np.hstack((np.linspace(0, 1, 10),np.linspace(1,6,16)[1:])) * 3.4/np.sqrt(2)
...
def run(fmag):
    ...
    #create integrator
    integrate = monolayer_integrator(G, G, post_callback=forcing, ndim=3, viewer=False, save_rate=-1, maxwell=False, minimal=True)
    t_final=5000

    pattern=f'data/elastic/Task2_{fmag}_{tau}_eta_{const.eta}_dt_{dt}_*.pickle'
    # pattern=None

    integrate(dt, t_final, adaptive=True, length_prec=0.01, save_pattern=pattern)
    #integrate
    # if fmag>=0:
