...
try:
    ...
    viewable=True
    base_path = './data/clinton_og'
except:
    viewable=False
    base_path = '/scratch/st-jjfeng-1/lmackay/data/clinton_og'
...
G.graph['basal_offset']=1000
G.graph['circum_sorted']=circum_sorted
G.graph['centers']=centers

if viewable:
    view = edge_viewer(G, apical_only=True, attr='myosin')
...
inter_edges = [[234,233], [36, 35], [224, 225], [29, 30]] #4
inter_edges.extend([[231,230],[228, 227], ]) #6
inter_edges.extend([[104, 103], [94, 95]]) #8
inter_edges.extend([[101, 168], [166, 97], [219, 221],  [210, 212]]) #10-12 


prefix = os.path.join(base_path, f'inter_{num_inter}')
...
#G = nx.read_gpickle('/scratch/st-jjfeng-1/cdurney/fly-glands-5/3D/t3099.pickle')
#
...
print(t) 
file_name = prefix + '_t_' + str(int(t)) 
...
while t <= const.t_final:
    ...
    # consider the inner arc part of the pit.  Therefore, it begins with the pit. 
    if t >= const.t_inner:
        for i in range(0,len(inner_arc)):
            G[inner_arc[i-1]][inner_arc[i]]['myosin'] = const.belt_strength   

    if t > const.t_intercalate:
        for e,c in zip(inter_edges, contract):
            if c:
                G[e[0]][e[1]]['myosin'] = const.intercalation_strength
                ...
                if (neighbor < 1000) and (neighbor not in belt) and (node not in centers) and (neighbor not in centers) and ([min(node, neighbor), max(node, neighbor)] not in blacklist): 
                    ...
                    if (dist < const.l_intercalation): 
                        if (np.random.rand(1)[0] < 1.):
                            ...
                            # apical 
                            # new edges 
                            G.add_edge(node,temp2[0],l_rest = const.l_apical, myosin=0.0, color='#808080')
                            G.add_edge(neighbor,temp1[0],l_rest = const.l_apical, myosin=0.0, color='#808080')
                            # new spokes 
                            G.add_edge(neighbor,ii,l_rest = const.l_apical, myosin=0.0)
                            G.add_edge(node,jj,l_rest = const.l_apical, myosin=0.0)
                            # basal 
                            # new edges 
                            G.add_edge(node+1000,temp2[0]+1000,l_rest = const.l_apical, myosin=0.0, color='#808080')
                            G.add_edge(neighbor+1000,temp1[0]+1000,l_rest = const.l_apical, myosin=0.0, color='#808080')
                            # new spokes 
                            G.add_edge(neighbor+1000,ii+1000,l_rest = const.l_apical, myosin=0.0)
                            G.add_edge(node+1000,jj+1000,l_rest = const.l_apical, myosin=0.0)
                            
                            # reset myosin on contracted edge
                            G[node][neighbor]['myosin'] = 0.0
                            G[node+1000][neighbor+1000]['myosin'] = 0.0
                            ...
                            if [node, neighbor] in inter_edges:
                                index = inter_edges.index([node, neighbor])
                            elif [neighbor, node] in inter_edges:
                                index = inter_edges.index([neighbor, node])

                            if index>=0 and index<num_inter:
...
# Save nx Graph in pickled form for plotting later
    if t % 1 == 0: 
        ...
        file_name = prefix + '_t_' + str(round(t)) 
        nx.write_gpickle(G,file_name + '.pickle')
        np.save(file_name,circum_sorted)
