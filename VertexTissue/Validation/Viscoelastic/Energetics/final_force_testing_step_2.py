...
from VertexTissue.TissueForces import bending_forces, compute_network_indices, myosin_forces, pressure_forces, spring_forces
...
def run(force, visco=False,  phi0=1.0, level=0, arcs=2, cable=True, continuous_pressure=False,
    ...
    def constant_pressure(i,j,locals=None, **kw):
        ...
        node_inds, cell_inds = cell_inds_touching_edge(i, j, centers)

        curr_vols = [convex_hull_volume_bis(get_points(G, c, pos) ) for c in node_inds]
        v0[cell_inds]=curr_vols+PI[cell_inds]/const.press_alpha
...
def cell_inds_touching_edge(G, i,j, centers):
        node_inds = list({ k for k in G.neighbors(i) if k in centers}.union( { k for k in G.neighbors(j) if k in centers}))
        cell_inds = [np.argwhere(centers == c)[0,0] for c in node_inds]
        return node_inds, cell_inds
...
def myosin_force_timeseries(d,phi0=1.0, summary=np.max, tmax=np.inf ,nodes=None ,**kw):
    ...
    for t,G in d.items():
        if t>tmax:
            break
        forces=np.sqrt(np.sum(myosin_forces(G)**2,axis=1))
...
def pressure_force_timeseries(d, summary=np.max, tmin=0, tmax=np.inf, nodes=None,**kw):
    ...
    for t,G in d.items():
        ...
        series.append((t,value))
    # f_vec = np.array([ (t,summary(np.sqrt(np.sum(pressure_forces(G, faces=faces, )**2,axis=1)))) for t,G in d.items() if t<tmax])


    return np.array(series)


def bending_force_timeseries(d, summary=np.max, tmin=0, tmax=np.inf, nodes=None,**kw):
    ...
    for t,G0 in d.items():
        if t>=tmin:
            ...
            break
    # G0=first_dict_value(d)
    _, _, shared_inds, alpha_inds, beta_inds, triangles_sorted, _= compute_network_indices(G0)
    triangulation = ( shared_inds, alpha_inds, beta_inds, triangles_sorted)
    forces=bending_forces(G0, triangulation=triangulation)
    forces_prev = forces
    # p0 = pressure_forces(G0,faces=faces)
    v0=const.v_0
    series=[]
    for t,G in d.items():
        forces_prev_prev = forces_prev
        forces_prev = forces
        if t>tmax:
            break
        if 'v0' in G.graph.keys():
        ...
        if t>tmin:
            ...
            forces=bending_forces(G, triangulation=triangulation)
            # print(np.abs(forces).max(),np.abs(forces+forces_prev).max()/2)
            dp=np.sqrt(np.sum(forces**2,axis=1))
...
def main():
    ...
    kws={'cable':[True,], 'level':1, 'visco':[True,], 'continuous_pressure':True, 'intercalations':1, 't_final':1000, 'pit_strength':540}
    kws0={'cable':[True,], 'level':1, 'visco':[True,], 'continuous_pressure':True, 'intercalations':0, 't_final':1000, 'pit_strength':540}

    from copy import deepcopy
    kws0=deepcopy(kws_middle)
    kws=deepcopy(kws_middle)
    # phi0=.3
    # kws0['phi0']=phi0
    # kws['phi0']=phi0
    kws0['intercalations']=0
    ...
    G, G_apical = tissue_3d( hex=7,  basal=True)
    # _ , nodes =  cell_inds_touching_edge(G, *inter_edges[0], G.graph['centers'])

    # nodes=[int(n) for n in nodes]

    # p0=sweep([1.0,], run_step4,  kw = kws0, pre_process=pressure_force_timeseries, pass_kw=True, 
    #     pre_process_kw={'summary':np.max,  'nodes':nodes}, savepath_prefix=base_path, 
    #     overwrite=False, cache=True, refresh=True)

    # r0=sweep([1.0,], run_step4,  kw = kws0, pre_process=spring_force_timeseries, pass_kw=True, 
    #     pre_process_kw={'summary':np.max, 'nodes':nodes}, savepath_prefix=base_path, 
    #     overwrite=False, cache=True, refresh=True)
    ...
    # # plt.savefig(f'max_forces_no_intercalations.pdf')
    # plt.show()
    G, G_apical = tissue_3d( hex=7,  basal=True)
    func = run_step2
    funcs={2:run_step2, 4:run_step4}
    phi0=1.0
    basal_offset=G.graph['basal_offset']
    for step, style in zip((2,),('-','--')):
        pressure=[]
        spring=[]
        myosin=[]
        for i in intercalations:
            kws['intercalations']=i


            inter_edges = get_inter_edges(intercalations=max(i,1), outer=False, double=False)

            nodes=[]
            cells=[]
            for e in inter_edges:
                
                ns, _ =  cell_inds_touching_edge(G, *e, G.graph['centers'])
                nodes.extend(ns )
                cells.extend(_)
            ...
            nodes=[int(n) for n in np.unique(np.array(G.graph['circum_sorted'])[cells])]
            
            nodes=[*nodes, *[int(n+basal_offset) for n in nodes] ]
            all_nodes=nodes

            nodes = [ int(n) for n in np.array(inter_edges).ravel()]
            nodes=[*nodes, *[int(n+basal_offset) for n in nodes] ]
            nodes=[inter_edges[-1][-1]+basal_offset]

            print(phi0)
            # p0=sweep([phi0,], func,  kw = kws0, pre_process=pressure_force_timeseries, pass_kw=True, 
            #     pre_process_kw={'summary':np.max,  'nodes':nodes, 'tmin':750}, savepath_prefix=base_path, 
            #     overwrite=False, cache=True, refresh=False)[0]

            # r0=sweep([phi0,], func,  kw = kws0, pre_process=spring_force_timeseries, pass_kw=True, 
            #     pre_process_kw={'summary':np.max, 'nodes':nodes, 'tmin':750}, savepath_prefix=base_path, 
            #     overwrite=False, cache=True, refresh=False)[0]
            ...
            # b=sweep([phi0,], func,  kw = kws, pre_process=bending_force_timeseries, pass_kw=True, 
            #     pre_process_kw={'summary':np.max, 'nodes':nodes, 'tmin':1000}, savepath_prefix=base_path, 
            #     overwrite=False, cache=True, refresh=False)[0]

            # m=sweep([phi0,], func,  kw = kws, pre_process=myosin_force_timeseries, pass_kw=True, 
            #     pre_process_kw={'summary':np.max, 'nodes':all_nodes}, savepath_prefix=base_path, 
            #     overwrite=False, cache=True, refresh=False)[0]

            pressure.append(np.mean(p[-6:,1]))
            spring.append(np.mean(r[-6:,1]))
            # myosin.append(np.mean(b[-6:,1]))
    ...
    plt.ylabel(r'max force $\left(\mu \rm{N} \right)$', fontsize=14)
    plt.xlabel('# of intercalations', fontsize=14)
    plt.xticks(intercalations)
    ylim=plt.ylim()
    plt.ylim((0,ylim[-1]))
    plt.tight_layout()
    plt.legend()
    # plt.savefig('final_forces_discontinuous_pressure.pdf')
    plt.show()
        # pmax = p[:,1].max()
        # jump = pmax>1.5*r[:,1].max()
        ...
        # # plt.savefig(f'max_forces_long_term_intercalations_{i}_step_{s}.pdf')
        # plt.show()
    ...
    #     p=sweep([1.0,], run_step2,  kw = kws, pre_process=pressure_force_timeseries, pass_kw=True, 
    #             pre_process_kw={'summary':np.max, 'tmin':750}, savepath_prefix=base_path, 
    #             overwrite=False, cache=True, refresh=False)


                
    #     r=sweep([1.0,], run_step2,  kw = kws, pre_process=spring_force_timeseries, pass_kw=True, 
    #         pre_process_kw={'summary':np.max, 'tmin':700}, savepath_prefix=base_path, 
    #         overwrite=False, cache=True, refresh=False)
    ...
    #     # plt.savefig(f'max_forces_intercalations_{i}_continuous_pressure_{c}.pdf')
    #     plt.show()
