...
def run(phi0, remodel=True, cable=True, L0_T1=0.0, verbose=False, belt=True, intercalations=0, 
        ...
        pit_strength=300, clinton_timestepping=False, dt_min=5e-2, basal=False, scale_pit=True):
    ...
    squeeze = SG.arcs_pit_and_intercalation(G, belt, t_1=t_start, inter_edges=inter_edges if not stochastic else [], 
...
sigma=np.linspace(0,600,20)

kws_baseline_pit = {'intercalations':0, 'L0_T1':L0_T1s, 'remodel':False , 'pit_strength':sigma}
...
if __name__ == '__main__':
    ...
    sweep([1.0,], run, kw=kws_baseline_pit, savepath_prefix=base_path, overwrite=False, pre_process=foo)
#     run(1.0, L0_T1=l_apical, intercalations=14, remodel=False,   verbose=True, viewable=True, outer=False, stochastic=False,  pit_strength=540, basal=False, dt_min=0.05)
